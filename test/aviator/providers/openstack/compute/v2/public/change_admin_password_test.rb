...
class Aviator::Test
  ...
  describe 'aviator/providers/openstack/compute/v2/public/change_admin_password' do

    def create_request(session_data = get_session_data, &block)
      block ||= lambda do |params|
                  params[:id]        = 0
                  params[:adminPass] = 'doesnotmatter'
                end
    ...
    def klass
      @klass ||= helper.load_request('openstack', 'compute', 'v2', 'public', 'change_admin_password.rb')
    end
    ...
    validate_response 'valid params are provided' do
      ...
      response = session.compute_service.request :change_admin_password do |params|
        params[:id]        = server_id
    ...
    validate_response 'invalid server id is provided' do
      ...
      response = session.compute_service.request :change_admin_password do |params|
        params[:id]        = server_id
        params[:adminPass] = 'it does not matter'
      end
