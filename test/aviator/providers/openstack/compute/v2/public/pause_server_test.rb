...
class Aviator::Test
  ...
  describe 'aviator/providers/openstack/compute/v2/public/pause_server' do
    ...
    def klass
      @klass ||= helper.load_request('openstack', 'compute', 'v2', 'public', 'pause_server.rb')
    end
    ...
    validate_response 'valid params are provided' do
      ...
      response = service.request :pause_server do |params|
        params[:id] = server[:id]
      end
    ...
    validate_response 'invalid server id is provided' do
      ...
      response = session.compute_service.request :pause_server do |params|
        params[:id] = server_id
      end
