...
class Aviator::Test
  ...
  describe 'aviator/openstack/volume/v2/admin/get_default_quotas' do
    ...
    def klass
      @klass ||= helper.load_request('openstack', 'volume', 'v2', 'admin', 'get_default_quotas.rb')
    end
    ...
    validate_attr :url do
      url    = "#{ v2_base_url }/os-quota-sets/#{ tenant_id }/defaults"
      tenant = tenant_id
    ...
    validate_response 'non-existent tenant is provided' do
      ...
      response.body.wont_be_nil
      response.body[:quota_set].wont_be_nil
      response.body[:quota_set][:id].must_equal tenant
      response.headers.wont_be_nil
    end
