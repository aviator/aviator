...
class Aviator::Test
  ...
  describe 'aviator/openstack/compute/v2/admin/delete_security_group' do
    ...
    def klass
      @klass ||= helper.load_request('openstack', 'compute', 'v2', 'admin', 'delete_security_group.rb')
    end
    ...
    validate_attr :url do
      ...
      sec_id       = 0
      url          = "#{ service_spec[:endpoints][0][:adminURL] }/os-security-groups/#{ sec_id }"
    ...
    validate_response 'non existent security group id is provided' do
      security_group  = session.compute_service.request(:list_security_groups).body[:security_groups].first
      security_group_id = security_group.nil? ? 0 : security_group[:id]
      security_group_id +=1
